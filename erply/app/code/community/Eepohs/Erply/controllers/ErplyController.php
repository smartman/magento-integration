...
//        Mage::helper('eepohs_erply')->log("Timestamp: ".$now);
        $minutes = date('i', $now);
        ...
        $scheduleDateTime = date('Y-m-d ' . $hours . ':' . $minutes . ':00',
            $now);
        $queueData["scheduleDateTime"] = $scheduleDateTime;
        /*
         * If there are any Queue's with same run code, then let's delete them
         */
        Mage::getModel('eepohs_erply/queue')->deleteQueueByCode('erply_' . $queueData["type"]);

        Mage::getModel('eepohs_erply/queue')->addQueue($queueData);
        Mage::getSingleton('core/session')->addSuccess(Mage::helper('eepohs_erply')->__('Import for %s has been scheduled at %s!',
                $queueData["type"], $queueData["scheduleDateTime"]));
        $this->_redirect('eepohs_erply/erply', $arguments = array());
    }
